...
function stopEditor() {
  let sheet = $("#stopEditorSheet");
  if (sheet) return sheet;
  document.body.insertAdjacentHTML(
    "beforeend",
    `<div id="stopEditorSheet" class="bottom-sheet" aria-hidden="true">
      <button class="sheet-scrim" data-close-stop-editor type="button" aria-label="關閉"></button>
      <form id="stopEditorForm" class="sheet-panel">
        <div class="sheet-grabber"></div>
        <div class="sheet-head">
          <div>
            <span class="mini-label">Itinerary</span>
            <h2 id="stopEditorTitle">修改行程</h2>
          </div>
          <button class="sheet-close" data-close-stop-editor type="button" aria-label="關閉">×</button>
        </div>
        <label>時間<input id="stopTimeInput" autocomplete="off" placeholder="例如 10:30-12:30" /></label>
        <label>地點 / 標題<input id="stopTitleInput" required autocomplete="off" placeholder="例如 teamLab Borderless" /></label>
        <label>行程內容<textarea id="stopDescriptionInput" rows="3" placeholder="簡短寫活動內容"></textarea></label>
        <label>備註<input id="stopNoteInput" autocomplete="off" placeholder="票券、價格、注意事項" /></label>
        <label>Google Map 搜尋地點<input id="stopMapInput" autocomplete="off" placeholder="例如 teamLab Borderless Azabudai Hills" /></label>
        <div class="sheet-actions">
          <button class="ghost-button" data-close-stop-editor type="button">取消</button>
          <button class="primary-button" type="submit">儲存行程</button>
        </div>
      </form>
    </div>`,
  );
  return $("#stopEditorSheet");
}

function openStopEditor(stop = ["", "", "", "", ""], title = "修改行程") {
  const sheet = stopEditor();
  const form = $("#stopEditorForm");
  const fields = {
    title: $("#stopEditorTitle"),
    time: $("#stopTimeInput"),
    name: $("#stopTitleInput"),
    description: $("#stopDescriptionInput"),
    note: $("#stopNoteInput"),
    map: $("#stopMapInput"),
  };
  fields.title.textContent = title;
  fields.time.value = stop[0] || "";
  fields.name.value = stop[1] || "";
  fields.description.value = stop[2] || "";
  fields.note.value = stop[3] || "";
  fields.map.value = stop[4] || stop[1] || "";
  ...
  return new Promise((resolve) => {
    const close = (value = null) => {
      ...
      sheet.querySelectorAll("[data-close-stop-editor]").forEach((button) => button.removeEventListener("click", onCancel));
      resolve(value);
    };
    ...
    const onSubmit = (event) => {
      ...
      close([
        ...
        fields.map.value.trim() || name || "Tokyo",
      ]);
    };
    form.addEventListener("submit", onSubmit);
    sheet.querySelectorAll("[data-close-stop-editor]").forEach((button) => button.addEventListener("click", onCancel));
  });
}
...
function renderPlan() {
  ...
  els.addStopBtn.onclick = async () => {
    const nextStop = await openStopEditor(["", "", "", "", ""], "新增行程");
    if (!nextStop) return;
    ...
    .map((stop, index) => {
      ...
      return `<article class="stop-card ${state.doneStops[key] ? "done" : ""}">
        ...
        <div class="stop-main">
          ...
          ${transportLink}
          <div class="stop-menu ${openStopMenu === key ? "open" : ""}">
            <button class="stop-menu-button" data-stop-menu="${key}" type="button" aria-label="行程操作">⋯</button>
            <div class="stop-menu-popover">
              <button data-edit-stop="${index}" type="button">修改行程</button>
              <button class="danger" data-delete-stop="${index}" type="button">刪除</button>
            </div>
          </div>
        </div>
  ...
  document.querySelectorAll("[data-stop-menu]").forEach((button) => {
    button.addEventListener("click", () => {
      openStopMenu = openStopMenu === button.dataset.stopMenu ? null : button.dataset.stopMenu;
      renderPlan();
    });
  });
  document.querySelectorAll("[data-edit-stop]").forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      const updatedStop = await openStopEditor(stops[index], "修改行程");
